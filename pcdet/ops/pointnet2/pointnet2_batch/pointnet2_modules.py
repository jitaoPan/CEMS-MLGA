...
class PointnetSAModuleMSG_WithSampling(_PointnetSAModuleBase):
    ...
    def __init__(self, *,
        ...
        #     4: 0.6
        # }
        # self.insample = pointnet2_utils.InstanceSampling(radius_dict, 0.1)
    ...
    def forward(self, xyz: torch.Tensor, features: torch.Tensor = None, cls_features: torch.Tensor = None, new_xyz=None,
        ...
        if ctr_xyz is None:
            ...
            for i in range(len(self.sample_type_list)):
                ...
                elif 'delete' in sample_type:
                    new_xyz, sample_idx = self.insample(xyz_tmp.contiguous(), cls_features_tmp)
                    sample_idx = sample_idx.int()
        ...
        if self.confidence_layers is not None:
            cls_features = self.confidence_layers(new_features)
            new_features = torch.cat((new_features, cls_features),dim=1)
...
class UpSampling_layer(nn.Module):
    def __init__(self, pre_channel, mlp_list, confidence_mlp, num_class):
        super().__init__()
        self.mlp_modules = nn.Sequential(
            nn.Conv1d(pre_channel, mlp_list[0], kernel_size=1, bias=False),
            nn.BatchNorm1d(mlp_list[0]),
            nn.ReLU()
        )

        self.confidence_layers = nn.Sequential(
            nn.Conv1d(mlp_list[0], confidence_mlp[0], kernel_size=1, bias=False),
            nn.BatchNorm1d(confidence_mlp[0]),
            nn.ReLU(),
            nn.Conv1d(confidence_mlp[0], num_class, kernel_size=1, bias=True),
        )

    def forward(self, old_xyz, xyz, old_features, features):
        """
        Args:
            old_xyz: (8,1024,3)
            xyz: (8,512,3)
            old_features: (8,131,1024)
            features: (8,256,512)
        Returns:
        """
        dist, idx = pointnet2_utils.three_nn(old_xyz, xyz)
        dist_recip = 1.0 / (dist + 1e-8)
        norm = torch.sum(dist_recip, dim=2, keepdim=True)
        weight = dist_recip / norm
        three_interpolate_features = pointnet2_utils.three_interpolate(features, idx, weight) # (8,256,1024)

        new_features = torch.cat((old_features, three_interpolate_features), dim=1)
        new_features = self.mlp_modules(new_features) # 128+256->256

        cls_features = self.confidence_layers(new_features)
        new_features = torch.cat((new_features, cls_features),dim=1)
        cls_features = cls_features.transpose(1, 2)
        return old_xyz, new_features, cls_features
...
class Vote_layer(nn.Module):
    ...
    def forward(self, xyz, features):
        ...
        # new_xyz, sample_idx = self.insample(vote_xyz.contiguous(), new_features)
        # sample_idx = sample_idx.int()
...
class SelfAttention(nn.Module):
    def __init__(self, in_dim, attention_dim):
        super().__init__()
        # position encoding
        self.pos_encod = nn.Sequential(nn.Conv1d(3, in_dim // 2, kernel_size=1, bias=False),
                                       nn.BatchNorm1d(in_dim // 2),
                                       nn.ReLU(),
                                       nn.Conv1d(in_dim // 2, in_dim, kernel_size=1, bias=False),
                                       )

        # Self attention layers
        self.self_attn = SelfAttentionBlock(in_dim=in_dim, attention_dim=attention_dim)

    def forward(self, xyz, features):
        xyz_trans = xyz.transpose(1, 2)
        pos_encod = self.pos_encod(xyz_trans)
        new_features = features + pos_encod
        attn_features = self.self_attn(new_features)
        attn_features = attn_features + features
        return xyz, attn_features


class SelfAttentionBlock(nn.Module):
    def __init__(self, in_dim, attention_dim, groups=4):
        super().__init__()
        self.groups = groups
        self.pre_norm = nn.LayerNorm(in_dim)
        self.query = nn.Conv1d(in_dim, attention_dim, kernel_size=1, bias=False)
        self.key = nn.Conv1d(in_dim, attention_dim, kernel_size=1, bias=False)
        self.value = nn.Conv1d(in_dim, attention_dim, kernel_size=1, bias=False)
        self.softmax = nn.Softmax(dim=-1)
        # self.fc = nn.Conv1d(attention_dim, in_dim, kernel_size=1, groups=self.groups, bias=False)
        self.fc = nn.Conv1d(attention_dim, in_dim, kernel_size=1, bias=False)

    def kernel(self, query, key, value):
    ...
    def forward(self, features):
        ...
        features_trans = self.pre_norm(features_trans)
        features_trans = features_trans.transpose(1, 2)

        query = self.query(features_trans)
        key = self.key(features_trans)
        value = self.value(features_trans)
        ...
        if self.groups and self.groups > 1:
            ...
            query_s = torch.split(query, split_size_or_sections=_c, dim=1)
            key_s = torch.split(key, split_size_or_sections=_c, dim=1)
            value_s = torch.split(value, split_size_or_sections=_c, dim=1)

            _t_sequences = []
            for i in range(self.groups):
                _x = self.kernel(query_s[i], key_s[i], value_s[i])
                _t_sequences.append(_x)
            x = torch.cat(_t_sequences, dim=1)
        else:
            x = self.kernel(query, key, value)
        x = self.fc(x)
        return x
...
class MultiLevelGraphAttention(nn.Module):
    def __init__(self, in_dim, attention_dim, k1, k2, m, num_class, 
                 aggregation_mlp=None, confidence_mlp=None):
        super().__init__()
        self.k1 = k1
        self.k2 = k2
        self.m = m
        ...
        # 注意力计算层
        self.query = nn.Linear(in_dim, attention_dim)
        self.key = nn.Linear(in_dim, attention_dim)
        self.value = nn.Linear(in_dim, attention_dim)
        
        # 注意力机制
        self.intra_attention = nn.MultiheadAttention(attention_dim, num_heads=4)
        self.cross_attention = nn.MultiheadAttention(attention_dim, num_heads=4)
        
        # 特征聚合MLP
        self.aggregation_mlp = self._build_mlp(aggregation_mlp)
        self.confidence_mlp = self._build_mlp(confidence_mlp)
    ...
    def _build_mlp(self, mlp_config):
        ...
        for i in range(len(mlp_config)-1):
            layers.append(nn.Linear(mlp_config[i], mlp_config[i+1]))
            if i < len(mlp_config)-2:
                layers.append(nn.ReLU())
        return nn.Sequential(*layers)
    
    def forward(self, xyz, features, prev_level_xyz=None, prev_level_features=None):
        """
        Args:
            xyz: (B, N, 3) 当前层点坐标
            features: (B, C, N) 当前层点特征
            prev_level_xyz: (B, M, 3) 前一层点坐标
            prev_level_features: (B, C, M) 前一层点特征
        Returns:
            li_xyz: (B, N, 3) 更新后的点坐标
            li_features: (B, C', N) 更新后的点特征
            li_cls_pred: (B, N, num_class) 分类预测
        """
        ...
        try:
            ...
            attn_output_g1, _ = self.intra_attention(
                query, key, value,
                key_padding_mask=None,
                attn_mask=None
            )
            attn_output_g1 = attn_output_g1.transpose(0, 1)
            if prev_level_features is not None and prev_level_xyz is not None:
                prev_level_features = prev_level_features.permute(0, 2, 1)
                if prev_level_features.shape[1] == 0:
                    cross_attn_output = torch.zeros_like(attn_output_g1)
                else:
                    cross_key = self.key(prev_level_features).transpose(0, 1)
                    cross_value = self.value(prev_level_features).transpose(0, 1)
                ...
                combined_features = torch.cat([attn_output_g1, cross_attn_output], dim=-1)
            else:
                combined_features = attn_output_g1
            if self.aggregation_mlp:
                combined_features = self.aggregation_mlp(combined_features)
            if self.confidence_mlp:
                cls_pred = self.confidence_mlp(combined_features)
                cls_pred = cls_pred.permute(0, 2, 1)
            else:
                cls_pred = None
            combined_features = combined_features.permute(0, 2, 1)
            return new_xyz, combined_features, cls_pred 
        except Exception as e:
            return xyz, old_features, None
