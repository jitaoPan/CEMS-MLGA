...
class SparseHEDNetWithCEMS(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, class_names, voxel_size, point_cloud_range, **kwargs):
        super().__init__()
        ...
        # CEMS模块参数
        self.cems_enabled = model_cfg.get('CEMS', False)
        self.cems_topk = model_cfg.get('CEMS_TOPK', 4096)
        self.cems_topk_select = model_cfg.get('CEMS_TOPK_SELECT', 2048)
        ...
        if self.cems_enabled:
            ...
            self.fusion_linear = nn.Linear(32, 16)
    ...
    def _voxel_coords_to_xyz(self, coords):
        return coords[:, [3, 2, 1]].float() * self.voxel_size.to(coords.device)

    def _apply_cems(self, x):
        if not self.cems_enabled:
            return x
        ...
        # 分类预测
        cls_logits = self.voxel_cls(x_conv1.features)
        cls_scores = self.softmax(cls_logits)
        max_scores, _ = cls_scores.max(dim=1)

        # 选择topk voxels
        topk = min(self.cems_topk, x_conv1.features.shape[0])
        _, indices_sorted = torch.topk(max_scores, topk)
        top_select = indices_sorted[:self.cems_topk_select]
        top_all = indices_sorted

        # 获取位置和特征
        pos_all = self._voxel_coords_to_xyz(x_conv1.indices)
        feat_all = x_conv1.features
        pos_top_all = pos_all[top_all]
        feat_top_all = feat_all[top_all]
        pos_top_select = pos_all[top_select]

        # 特征增强
        enhanced_feat = []
        for i in range(pos_top_select.shape[0]):
            dists = torch.norm(pos_top_all - pos_top_select[i], dim=1)
            top3_idx = torch.topk(-dists, 3).indices
            interp_feat = feat_top_all[top3_idx].mean(dim=0)
            enhanced_feat.append(interp_feat)
        enhanced_feat = torch.stack(enhanced_feat, dim=0)

        # 特征融合
        original_feat = feat_all[top_select]
        fused_feat = self.fusion_linear(torch.cat([original_feat, enhanced_feat], dim=1))
        feat_all = x_conv1.features.clone()
        feat_all[top_select] = fused_feat

        return x_conv1.replace_feature(feat_all)
    ...
    def assign_target(self, batch_spatial_indices, batch_gt_boxes):
        all_names = np.array(['bg', *self.class_names])
        inside_box_target = batch_spatial_indices.new_zeros(
    ...
    def feature_diffusion(self, x, batch_dict):
        ...
        for gidx, inside_mask in enumerate(group_inside_mask):
            ...
            pooling_size = self.group_pooling_kernel_size[gidx]
            single_one_mask = F.max_pool2d(single_one_mask, kernel_size=pooling_size, stride=1,
