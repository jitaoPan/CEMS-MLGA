...
class AnchorHeadSingleWithMLGA(AnchorHeadTemplate):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range,
        ...
        self.mlga_enabled = model_cfg.get('MLGA', {}).get('ENABLED', False)
        if self.mlga_enabled:
            self._build_mlga_module(model_cfg.MLGA, input_channels)
    ...
    def _build_mlga_module(self, mlga_cfg, input_channels):
        """Build Multi-Level Graph Attention components"""
    ...
    def _mlga_forward(self, spatial_features_2d, batch_dict):
        """
        Multi-Level Graph Attention forward pass
        Args:
            spatial_features_2d: (B, C, H, W) feature map
            batch_dict: Contains point cloud data
        Returns:
            enhanced_features: (B, C, H, W) enhanced features
