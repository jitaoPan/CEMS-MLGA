...
class IASSD_Head(PointHeadTemplate):
    ...
    def assign_stack_targets_IASSD(self, points, gt_boxes, extend_gt_boxes=None, weighted_labels=False,
        ...
        for k in range(batch_size):            
            bs_mask = (bs_idx == k) #(2048) (0,0,...,7,7,7..)
            points_single = points[bs_mask][:, 1:4] #(256,3)
            point_cls_labels_single = point_cls_labels.new_zeros(bs_mask.sum()) #(256,)
            box_idxs_of_pts = roiaware_pool3d_utils.points_in_boxes_gpu(
                points_single.unsqueeze(dim=0), gt_boxes[k:k + 1, :, 0:7].contiguous()
            ).long().squeeze(dim=0) #(256,) [-1,-1,...,34,-1,...,10,...]
            box_fg_flag = (box_idxs_of_pts >= 0) #(256,) [0,0,...,1,0,..,1,...]
            ...
            if use_query_assign: #False
                ...
                extend_box_idxs_of_pts = roiaware_pool3d_utils.points_in_boxes_gpu(
                    points_single.unsqueeze(dim=0), extend_gt_boxes[k:k+1, :, 0:7].contiguous()
                ).long().squeeze(dim=0) #(256,)
                fg_flag = box_fg_flag
                ignore_flag = fg_flag ^ (extend_box_idxs_of_pts >= 0) #yihuo
            ...
            gt_box_of_fg_points = gt_boxes[k][box_idxs_of_pts[fg_flag]] # box_idxs_of_pts[fg_flag] (31,)
        ...
        gt_boxes_of_fg_points = torch.cat(gt_boxes_of_fg_points, dim=0) #(245,8) 前景点的gt_boxes
        targets_dict = {
    ...
    def assign_targets(self, input_dict):
        ...
        targets_dict_center['center_gt_box_of_fg_points'] = center_targets_dict['gt_box_of_fg_points'] #(245,8)
        targets_dict_center['center_cls_labels'] = center_targets_dict['point_cls_labels'] #(2048,)
        targets_dict_center['center_box_labels'] = center_targets_dict['point_box_labels'] #only center assign (2048,8)
        targets_dict_center['center_gt_box_of_points'] = center_targets_dict['gt_box_of_points'] #(2048,8)
        if target_cfg.get('INS_AWARE_ASSIGN', False): #True
            sa_ins_labels, sa_gt_box_of_fg_points, sa_xyz_coords, sa_gt_box_of_points, sa_box_idxs_labels = [],[],[],[],[]
            sa_ins_preds = input_dict['sa_ins_preds'] # list:6 [[],[8,1024,4(bs+xyz)],[8,512,4],[],[],[]]
            for i in range(1, len(sa_ins_preds)): # valid when i = 1,2 for IA-SSD
                # if sa_ins_preds[i].__len__() == 0:
                #     continue
                sa_xyz = input_dict['encoder_coords'][i] #(8,4096,4)
                if i == 1:
                    extend_gt_boxes = box_utils.enlarge_box3d(
    ...
    def forward(self, batch_dict):
        ...
        center_features = batch_dict['centers_features']  #(8*256,512)
        center_coords = batch_dict['centers']  #(8*256,4)
        center_cls_preds = self.cls_center_layers(center_features)#(8*256,3)  (total_centers, num_class)
        center_box_preds = self.box_center_layers(center_features)#(8*256,30)  (total_centers, box_code_size)
